  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> _widgetOptions = <Widget>[
      ...
      SafeArea(
        child: Column(
          children: <Widget>[
            if (!_horoscopeDetailsSubmitted)
              Expanded(
                child: SingleChildScrollView(
                  padding: const EdgeInsets.all(16.0),
                  child: Column( // Main column for the form content
                    ...
                    children: [
                      Padding(
                        ...
                        child: Column( // Column to hold image and text
                            ...
                            children: [
                              Center( // Wrap SizedBox with Center to center the image
                                child: SizedBox(
                                  ...
                                  color: screenBackgroundColor, // Page background color
                                    ...
                                    alignment: Alignment.center, // Image is centered within SizedBox
                                  ),
                                ),
                              ),
                              ...
                              Text(
                                'Financial Education \n in detail...',
                                textAlign: TextAlign.left, // Already left, but good to confirm
                                style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                  fontSize: 20,
                                  height: 1.0,
                      ...
                      // Removed the extra horizontal Padding wrapper for form fields
                      Column( // Column for form fields, checkbox, and button
                          crossAxisAlignment: CrossAxisAlignment.start, // Align children to the start (left)
                          children: [ // Added filled: true and fillColor: Colors.white to all TextFields and Dropdowns
                            TextField(controller: _personNameController, decoration: const InputDecoration(labelText: 'Person Name', border: OutlineInputBorder(), contentPadding: EdgeInsets.symmetric(vertical: 8.0, horizontal: 12.0), filled: true, fillColor: Colors.white)),
                            ...
                            TextFormField(
                              ...
                              onTap: () => _selectDate(context, _dobController), // Also open picker on tap
                              validator: (value) {
                            ...
                            TextFormField(
                              ...
                              decoration: const InputDecoration(labelText: 'Time of Birth (e.g., 05:10 AM)', border: OutlineInputBorder(), contentPadding: EdgeInsets.symmetric(vertical: 8.0, horizontal: 12.0), filled: true, fillColor: Colors.white),
                              validator: (value) {
                                if (value == null || value.isEmpty) {
                                  return 'Please enter a time of birth.';
                                }
                                return null;
                              },
                            ),
                            const SizedBox(height: 12),
                            TextField(controller: _pobController, decoration: const InputDecoration(labelText: 'Place of Birth (e.g., City, State, Country)', border: OutlineInputBorder(), contentPadding: EdgeInsets.symmetric(vertical: 8.0, horizontal: 12.0), filled: true, fillColor: Colors.white)),
                            const SizedBox(height: 20),
                            Row(
                              crossAxisAlignment: CrossAxisAlignment.center, // Vertically align items
                              children: <Widget>[
                                ElevatedButton(
                                  onPressed: _horoscopeIsLoading ? null : _initiateHoroscopeChat,
                                  style: ElevatedButton.styleFrom(
                                    backgroundColor: buttonColor, // Button color
                                    padding: const EdgeInsets.symmetric(horizontal: 15, vertical: 11.25), // Reduced horizontal padding
                                  ),
                                  child: const Text('Start Session', style: TextStyle(color: Colors.white)),
                                ),
                                const SizedBox(width: 5), // Reduced spacing between button and checkbox
                                Checkbox(
                                  value: _horoscopeIncludeTodaysFortune,
                                  onChanged: (bool? value) {
                                    setState(() {
                                      _horoscopeIncludeTodaysFortune = value ?? false;
                                    });
                                  },
                                  activeColor: selectedTabColor, // Checkbox active color
                                  visualDensity: VisualDensity.compact, // Makes checkbox slightly smaller
                                ),
                                InkWell( // Makes the label tappable
                                  onTap: () {
                                    setState(() {
                                      _horoscopeIncludeTodaysFortune = !_horoscopeIncludeTodaysFortune;
                                    });
                                  },
                                  child: const Text("Today's Highligts"),
                                ),
                              ],
                            ),
                            if (_horoscopeIsLoading)
                              const Padding(
                                padding: EdgeInsets.all(8.0),
                                child: CircularProgressIndicator()), // Add comma for list item
                            if (_horoscopeErrorMessage.isNotEmpty) Padding(padding: const EdgeInsets.all(8.0), child: Text(_horoscopeErrorMessage, style: const TextStyle(color: Colors.red), textAlign: TextAlign.center)),
                          ],
                      ),
                    ],
                  ),
              ...
              Expanded(
                child: Column(
                  children: [
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                                  Text('Horoscope For:', style: Theme.of(context).textTheme.titleSmall),
                                  PopupMenuButton<String>(
                                    icon: const Icon(Icons.more_vert),
                                    onSelected: (String result) {
                                      if (result == 'restart') {
                                        _restartHoroscopeSession();
                                      } else if (result == 'download_pdf') {
                                        _generateHoroscopePdf();
                                      }
                                    },
                                    itemBuilder: (BuildContext context) => <PopupMenuEntry<String>>[
                                      ...
                                      PopupMenuItem<String>(
                                        value: 'download_pdf',
                                        enabled: !_isGeneratingHoroscopePdf && _horoscopeChatMessages.isNotEmpty,
                                        child: Text(_isGeneratingHoroscopePdf ? 'Generating...' : 'Download Chat PDF'),
                                      ),
                                    ],
                                  ),
                                ],
                              ...
                              Text('Name: ${_horoscopeSubmittedPersonName}', style: Theme.of(context).textTheme.bodyMedium),
                              Text('DOB: ${_horoscopeSubmittedDob}', style: Theme.of(context).textTheme.bodyMedium),
                              Text('TOB: ${_horoscopeSubmittedTob}', style: Theme.of(context).textTheme.bodyMedium),
                              Text('POB: ${_horoscopeSubmittedPob}', style: Theme.of(context).textTheme.bodyMedium),
                            ],
                          ),
                        ),
                      ),
                    ...
                    Expanded(
                      child: _horoscopeChatMessages.isEmpty && !_horoscopeIsLoading && _horoscopeErrorMessage.isEmpty
                          ? Center(
                              child: Text(
                          ...
                          : ListView.builder(
                              controller: _horoscopeChatScrollController,
                              padding: const EdgeInsets.all(8.0),
                              itemCount: _horoscopeChatMessages.length,
                              itemBuilder: (context, index) {
                                final message = _horoscopeChatMessages[index];
                                return _buildChatMessageBubble(message, "en-US"); // Horoscope defaults to English for TTS
                              },
                            ),
                    ),
                    if (_horoscopeIsLoading) const Padding(padding: EdgeInsets.symmetric(vertical: 8.0), child: CircularProgressIndicator()),
                    if (_horoscopeErrorMessage.isNotEmpty && !_horoscopeIsLoading) Padding(padding: const EdgeInsets.all(8.0), child: Text(_horoscopeErrorMessage, style: const TextStyle(color: Colors.red), textAlign: TextAlign.center)),
                    Padding(
                      padding: const EdgeInsets.all(8.0),
                      child: Row(
                        children: <Widget>[
                          Expanded(
                            child: TextField(
                              controller: _horoscopeChatInputController,
                              decoration: InputDecoration(
                              ...
                              onSubmitted: (value) {
                                if (value.isNotEmpty) _sendHoroscopeApiRequest(value);
                              },
                            ),
                          ),
                          IconButton(
                            ...
                            onPressed: () {
                              final text = _horoscopeChatInputController.text;
                              if (text.isNotEmpty) _sendHoroscopeApiRequest(text);
                            },
                          ),
                        ],
                      ),
      ...
      SafeArea(
        child: Column(
          children: <Widget>[
            if (!_mantraDetailsSubmitted)
              Expanded(
                child: SingleChildScrollView(
                  padding: const EdgeInsets.all(16.0),
                  child: Column(
                    children: [
                      // Updated Header Section for Mantra Tab
                      Padding(
                        ...
                        child: Row(
                          ...
                          children: <Widget>[
                            ...
                            Expanded(
                              child: Text(
                                'Daily Mantras for\nMind, Body & Soul...', // New text
                                textAlign: TextAlign.left, // Consistent text alignment
                                style: TextStyle( // Restored font size to match other tabs
                                  fontWeight: FontWeight.bold,
                                  fontSize: 20, // Consistent with other tab titles
                                  height: 1.0,
                                  letterSpacing: 0.0,
                                  color: Colors.brown[800],
                                ),
                            ...
                            // Image on the right - Increased height by 25%
                            SizedBox(
                              height: 156.25, // 125.0 * 1.25 = 156.25
                              child: Image.asset(
                                'assets/images/mantra-img.png',
                                fit: BoxFit.contain, // Ensure fit is appropriate
                      ...
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            TextField( // Added filled: true and fillColor: Colors.white
                              controller: _mantraQueryController,
                              decoration: const InputDecoration(labelText: 'Enter mantra keywords...', border: OutlineInputBorder(), contentPadding: EdgeInsets.symmetric(vertical: 8.0, horizontal: 12.0), filled: true, fillColor: Colors.white),
                            ),
                            const SizedBox(height: 12),
                            DropdownButtonFormField<String>( 
                              decoration: const InputDecoration(
                              ...
                              value: _selectedMantraResponseLanguage,
                              items: _selectableApiLanguages.map((String value) {
                                return DropdownMenuItem<String>(value: value, child: Text(value));
                              }).toList(),
                              onChanged: (String? newValue) {
                                if (newValue != null) {
                                  setState(() => _selectedMantraResponseLanguage = newValue);
                                }
                              },
                              dropdownColor: Colors.white, // Explicitly set dropdown background
                            ),
                            const SizedBox(height: 20),
                            ElevatedButton(
                              onPressed: _mantraIsLoading ? null : _initiateMantraChat,
                              style: ElevatedButton.styleFrom(
                                backgroundColor: buttonColor,
                              ),
                              child: const Text('Find Mantra & Chat', style: TextStyle(color: Colors.white)),
                            ),
                            if (_mantraIsLoading)
                              const Padding(
                                padding: EdgeInsets.all(8.0),
                                child: CircularProgressIndicator()),
                            if (_mantraErrorMessage.isNotEmpty)
                              Padding(padding: const EdgeInsets.all(8.0), child: Text(_mantraErrorMessage, style: const TextStyle(color: Colors.red), textAlign: TextAlign.center)),
                            _buildPopularMantras(),
                          ],
                        ),
                      ),
                    ],
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                                  Text('Mantra Session:', style: Theme.of(context).textTheme.titleSmall),
                                  PopupMenuButton<String>(
                                    icon: const Icon(Icons.more_vert),
                                    onSelected: (String result) {
                                      if (result == 'restart') {
                                        _restartMantraSession();
                                      } else if (result == 'download_pdf') {
                                        _generateMantraPdf();
                                      }
                                    },
                                    itemBuilder: (BuildContext context) => <PopupMenuEntry<String>>[
                                      const PopupMenuItem<String>(value: 'restart', child: Text('Restart Session')),
                                      PopupMenuItem<String>(value: 'download_pdf', enabled: !_isGeneratingMantraPdf && _mantraChatMessages.isNotEmpty, child: Text(_isGeneratingMantraPdf ? 'Generating...' : 'Download Chat PDF')),
                                    ],
                                  ),
                                ],
                              ),
                              const SizedBox(height: 4),
                              Text('Query: $_mantraSubmittedQuery', style: Theme.of(context).textTheme.bodyMedium),
                              Text('Language: $_mantraSubmittedResponseLanguage', style: Theme.of(context).textTheme.bodyMedium),
                            ],
                          ),
                        ),
                      ),
                    ),
                    if (_mantraYoutubeVideoId != null) _buildMantraYoutubePlayer(),
                    Expanded(
                      child: _mantraChatMessages.isEmpty && !_mantraIsLoading && _mantraErrorMessage.isEmpty
                          ? Center(child: Text('Mantra details will appear here...', style: TextStyle(fontSize: 16, color: Colors.grey), textAlign: TextAlign.center))
                          : ListView.builder(
                              controller: _mantraChatScrollController,
                              padding: const EdgeInsets.all(8.0),
                              itemCount: _mantraChatMessages.length,
                              itemBuilder: (context, index) {
                                final message = _mantraChatMessages[index];
                                return _buildChatMessageBubble(message, _currentMantraSessionTtsCode);
                              },
                            ),
                    ),
                    if (_mantraIsLoading) const Padding(padding: EdgeInsets.symmetric(vertical: 8.0), child: CircularProgressIndicator()),
                    if (_mantraErrorMessage.isNotEmpty && !_mantraIsLoading) Padding(padding: const EdgeInsets.all(8.0), child: Text(_mantraErrorMessage, style: const TextStyle(color: Colors.red), textAlign: TextAlign.center)),
                    Padding(
                      padding: const EdgeInsets.all(8.0),
                      child: Row(children: <Widget>[ // Added filled: true and fillColor: Colors.white
                        Expanded(child: TextField(controller: _mantraChatInputController, decoration: InputDecoration(hintText: 'Ask a follow-up question...', border: OutlineInputBorder(borderRadius: BorderRadius.circular(25.0)), contentPadding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 12.0), filled: true, fillColor: Colors.white), onSubmitted: (value) { if (value.isNotEmpty) _sendMantraApiRequest(value); })),
                        IconButton(icon: const Icon(Icons.send), onPressed: () { final text = _mantraChatInputController.text; if (text.isNotEmpty) _sendMantraApiRequest(text); })]),
                    ),
                  ],
                ),
              ),
